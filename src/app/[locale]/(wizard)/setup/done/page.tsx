...
export default async function SetupDonePage({
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-bg-base px-6 text-center">
        ...
        >
          <defs>
            <linearGradient id="grad-done" x1="0%" y1="0%" x2="100%" y2="100%">
              ...
              <stop offset="100%" style={{ stopColor: "var(--color-navy)" }} />
            </linearGradient>
          </defs>
          <rect x="2" y="6" width="36" height="32" rx="10" fill="url(#grad-done)" />
          <polygon points="8,6 4,0 12,0" fill="var(--color-violet)" />
          <polygon points="32,6 28,0 36,0" fill="var(--color-violet)" />
          <ellipse cx="14" cy="17" rx="2.5" ry="1.5" fill="#FFFFFF" opacity="0.9" />
          <ellipse cx="26" cy="17" rx="2.5" ry="1.5" fill="#FFFFFF" opacity="0.9" />
          <path d="M18.5 22 L20 23.5 L21.5 22 L20 21 Z" fill="var(--color-violet)" />
          <rect x="6" y="29" width="28" height="3.5" rx="1.75" fill="var(--color-violet)" />
          <path
      ...
      <p className="mt-3 max-w-sm text-text-muted">{t("subtitle")}</p>

      <DoneRedirect locale={locale} />
    </div>
  );
}
